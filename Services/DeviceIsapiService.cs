...
public sealed class DeviceIsapiService : IDisposable
{
    ...
    public async Task<string> GetCallerInfoAsync(CancellationToken token = default)
    {
        using var response = await _http.GetAsync("/ISAPI/VideoIntercom/callerInfo?format=json", token);
        return await response.Content.ReadAsStringAsync(token);
    }
