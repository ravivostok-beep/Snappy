...
public sealed class HikvisionIsapiService : IDisposable
{
    ...
    public HikvisionIsapiService(string ip, int port, string username, string password)
    {
    ...
    public async Task<string> GetCallStatusAsync(CancellationToken token = default)
    {
        using var response = await _http.GetAsync(
            "/ISAPI/VideoIntercom/callStatus?format=json", token);
        return await response.Content.ReadAsStringAsync(token);
    }

    public Task<string> AnswerAsync(CancellationToken token = default) =>
        SendCallSignalAsync("answer", token);

    public Task<string> RejectAsync(CancellationToken token = default) =>
        SendCallSignalAsync("reject", token);

    public Task<string> HangUpAsync(CancellationToken token = default) =>
        SendCallSignalAsync("hangUp", token);
        ...
        string command, CancellationToken token)
    {
        ...
        {
            CallSignal = new { cmdType = command }
        });
